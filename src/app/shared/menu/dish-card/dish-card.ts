...
@Component({
  ...
  imports: [AppImage, Price, QuantityControl],
  styles: `
    ...
    app-image { inline-size: 6.25rem; block-size: 6.25rem; border-radius: .75rem; }
  ...
  template: `
    <article (click)="detailsRequested.emit()">
      @if (dish().image; as image) { <app-image [alt]="name()" [height]="image.height" [src]="image.url" [width]="image.width" /> }
      <div>
        <h3>{{ name() }}</h3>
        <p>{{ description() }}</p>
        <div class="bottom">
